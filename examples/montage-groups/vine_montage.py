...
config = Config(
        executors=[TaskVineExecutor(
            manager_config=TaskVineManagerConfig(tune_parameters={"task-groups":0, "wait-for-workers":50}, init_command="export PATH=$PATH:$MONTAGE_BIN", port=9123, project_name="parsl_vine_montage"),
            factory_config=TaskVineFactoryConfig(batch_type="condor", min_workers=50, max_workers=50, workers_per_cycle=200, cores=12, disk=30000),
...
with parsl.load(config) as conf:
    ...
    for uid in uid_set:
        #kimages
        f_input_metadata = File(f"taskvinetemp://{uid}-Kimages.tbl")
        f_fits_header = File(f"taskvinetemp://{uid}-Ktemplate.hdr")
        f_imglist = File(f"{uid}-imglist")
        f_repro_imglist = File(f"{uid}-repro_imglist")

        #kprojdir
        f_input_projection_stats = File(f"taskvinetemp://{uid}-Kstats.tbl")
        f_overlap_diffs = File(f"taskvinetemp://{uid}-diffs.tbl")
        f_repro_metadata = File(f"taskvinetemp://{uid}-images.tbl")
        
        #diffdir
        f_fits_table = File(f"taskvinetemp://{uid}-fits.tbl")
        f_corrections_table = File(f"taskvinetemp://{uid}-corrections.tbl")
        f_mosaic = File(f"taskvinetemp://{uid}-m17.fits")
        ...
        for f in list_fits_input_names:
            f_strp = f.strip('./')
            list_repro_fits_inputs.append(File("taskvinetemp://hdu0_" + f_strp))
            f_area = "taskvinetemp://hdu0_" + f_strp.split(".fits")[0] + "_area.fits"
        ...
        with open('diff_files', 'r') as f:
            for l in f:
                l = l.strip('\n')
                list_diff_files.append(File(f"taskvinetemp://{uid}-" + l))
